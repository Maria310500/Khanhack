...
function setupMenu() {
    ...
    function setupWatermark() {
        ...
        watermark.innerHTML = `<span style="text-shadow: -1px 0.5px 0 #72ff72, -2px 0px 0 #2f672e;">Painel ne pae</span> <span style="color:gray; padding-left:2px; font-family: Arial, sans-serif; font-size:10px">${ver}</span>`;
    ...
    function setupDropdown() {
        ...
        let featuresList = [
            [{ name: 'questionSpoof', type: 'checkbox', variable: 'features.questionSpoof', attributes: 'checked', labeled: true, label: 'Pular Questão' },
            { name: 'videoSpoof', type: 'checkbox', variable: 'features.videoSpoof', attributes: 'checked', labeled: true, label: 'Pular Video' },
            ...
            { name: 'repeatQuestion', className: 'repeatQuestion', type: 'checkbox', variable: 'features.repeatQuestion', attributes: 'style="display:none;"', labeled: true, label: 'Repedir Questão' },
            ...
            { name: 'customBanner', type: 'checkbox', variable: 'features.customBanner', labeled: true, label: 'Banner Custom' },
            ...
            { name: 'onekoJs', type: 'checkbox', variable: 'features.onekoJs', labeled: true, label: 'Gatin' }]
        ]
        if (!device.apple) {
            featuresList.push(
                ...
                [{ name: 'Custom Imagem', type: 'nonInput' }, { name: 'customPfp', type: 'text', variable: 'featureConfigs.customPfp', attributes: 'autocomplete="off"' }]
            );
        }
    ...
    function setupStatusPanel() {
        ...
        const update = async () => statsPanel.innerHTML = `
            <span style="text-shadow: -1px 0.5px 0 #p, -2px 0px 0 #2f672e;">Painel ne pae</span>
...
function setupMain(){
    function spoofQuestion() {
        const phrases = [ "Feito por Cordeiro 🐑", "Feito por Cordeiro 🐑", "Feito por Cordeiro 🐑" ];
            ...
            try {
                ...
                if (features.questionSpoof && responseObj?.data?.assessmentItem?.item?.itemData) {
                    ...
                    if(itemData.question.content[0] === itemData.question.content[0].toUpperCase()){
                        ...
                        sendToast(" Questão Desbloqueada. 🔓", 1000);
            ...
            if (features.videoSpoof && body && body.includes('"operationName":"updateUserVideoProgress"')) {
                try {
                    ...
                    if (bodyObj.variables && bodyObj.variables.input) {
                        ...
                        sendToast(" Vídeo Desbloqueado.🔓", 1000)
                    }
                } catch (e) { }
            }
                                    ...
                                    if (widget.options && widget.options.choices) {
                                        widget.options.choices.forEach(choice => {
                                            if (choice.correct) {
                                                ...
                                                sendToast(" Respostas Amostras.🔓", 1000);                
                                            }
                                        });
                                    }
                                });
    ...
    function changeBannerText() {
        const phrases = [ "[🐑] Feito por Cordeiro", "[🐑] Feito por Cordeiro", "[🐑] Feito por Cordeiro", "[🐑] Feito por Cordeiro", "[🐑] Feito por Cordeiro", "[🐑] Feito por Cordeiro" ];
        setInterval(() => { 
